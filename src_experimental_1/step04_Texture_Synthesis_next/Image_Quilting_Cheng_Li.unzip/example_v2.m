...
dirWithTextures='/home/ar/dev-git.git/dev.python/PRJ_DATAMOLA/data/shirts.unzip_textures';
dirOutput=sprintf('%s-results', dirWithTextures);
...
lstfimg = dir([dirWithTextures, '/*.png']);
numImages = numel(lstfimg);
for ffi=1:numImages
    fimg=sprintf('%s/%s', dirWithTextures, lstfimg(ffi).name);
    texture = imread(fimg);
% % figure;

    ptrFig = figure('units','normalized','outerposition',[0 0 1 1]);
    ...
    tfout=sprintf('%s/%s-fig.jpg', dirOutput, lstfimg(ffi).name);
    saveas(ptrFig, tfout);
end
